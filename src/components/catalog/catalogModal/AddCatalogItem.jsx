...
const AddCatalogItem = ({ open, onClose }) => {
      ...
      >
        <form onSubmit={handleSubmit} className="flex h-full min-h-0 flex-col">
          <div className="shrink-0 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-gray-900">Add Product to Catalogue</h2>
              <p className="mt-1 text-sm text-gray-500">Add a new product for quick order entry.</p>
            </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5 custom-scrollbar">
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <Input name="name" value={formData.name} onChange={handleChange} placeholder="e.g. Standard Business Card" required />
              </div>

              <div className="space-y-1.5">
                <label className="label">SKU</label>
                <Input name="sku" value={formData.sku} onChange={handleChange} placeholder="e.g. BC-01" />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input name="retailPrice" type="number" min="0" step="0.01" value={formData.retailPrice} onChange={handleChange} placeholder="0.00" required />
              </div>

              <div className="space-y-1.5">
                <label className="label">Dealer Price <span className="required">*</span></label>
                <Input name="dealerPrice" type="number" min="0" step="0.01" value={formData.dealerPrice} onChange={handleChange} placeholder="0.00" required />
              </div>
              
              <div className="space-y-1.5">
                <label className="label">Tax Rate (%)</label>
                <Input name="taxPercent" type="number" min="0" step="0.1" value={formData.taxPercent} onChange={handleChange} placeholder="e.g. 18" />
              </div>

              <div className="space-y-1.5">
                <label className="label">Size</label>
                <Input name="size" value={formData.size} onChange={handleChange} placeholder="e.g. 3.5x2 inch" />
              </div>

              <div className="space-y-1.5">
                <label className="label">Color</label>
                <Input name="color" value={formData.color} onChange={handleChange} placeholder="e.g. CMYK" />
              </div>
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <Input type="textarea" name="description" value={formData.description} onChange={handleChange} placeholder="Any details..." className="min-h-[80px]" />
              </div>
          ...
          <div className="shrink-0 border-t border-gray-200 bg-gray-50/50 px-6 py-4 flex justify-end gap-3">
            ...
            <Button variant="solid" type="submit" isLoading={isSubmitting} disabled={isSubmitting || uploadingImage}>Save Product</Button>
          </div>
        </form>
      </div>
    </div>
