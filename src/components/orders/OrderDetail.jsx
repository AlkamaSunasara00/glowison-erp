...
const OrderDetail = ({ open, onClose, order, isPage = false, onOrderUpdated }) => {
  ...
  const detailPanelContent = (
    <div
      className={isPage ? "flex flex-col w-full min-h-screen bg-transparent animate-fade-in" : `relative flex h-full w-full max-w-full flex-col bg-white shadow-2xl md:w-[90%] md:h-screen ${open ? "animate-slide-in-right" : "animate-slide-out-right"
        }`}
      onClick={(e) => e.stopPropagation()}
    >
      {/* ── HEADER ─────────────────────────────────────────── */}
      <div className={isPage ? "block sm:flex sm:items-center sm:justify-between pb-6" : "block sm:flex sm:items-center sm:justify-between px-4 sm:px-7 py-4 border-b border-gray-100 bg-transparent"}>
        <div className="flex items-center gap-4 mb-4 sm:mb-0">
          ...
          <div>
            <h2 className="page-header flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className="flex-1 sm:flex-none justify-center rounded-lg px-3! py-1.5! text-xs font-medium"
          >
            Edit Order
          </Button>
          <Button
            ...
            className="flex-1 sm:flex-none justify-center rounded-lg px-3! py-1.5! text-xs font-semibold"
          >
            Generate Invoice
          </Button>
        </div>
      ...
      <div className={isPage ? "pb-10" : "flex-1 overflow-hidden"}>
        <div className={isPage ? "grid grid-cols-1 lg:grid-cols-12 gap-6" : "grid h-full grid-cols-1 lg:grid-cols-[1fr_340px] divide-y lg:divide-y-0 lg:divide-x divide-gray-100"}>
          ...
          <div className={isPage ? "lg:col-span-8 flex flex-col gap-6" : "h-full overflow-y-auto px-7 py-6 space-y-7 bg-gray-50/40"}>
            ...
            <section className={isPage ? "bg-transparent border border-gray-100/80 rounded-sm shadow-sm relative overflow-hidden group" : "mx-7 mb-7 bg-transparent border border-gray-200 rounded-sm p-6 shadow-sm"}>
              ...
              <div className="rounded-sm border border-gray-100 overflow-x-auto w-full custom-scrollbar">
                <table className="w-full text-left border-collapse whitespace-nowrap min-w-[600px]">
                  <thead className="bg-primary border-b border-primary/20 text-xs font-semibold text-white">
                    <tr>
                      <th className="px-2 sm:px-4 py-3 w-16">Image</th>
                      <th className="px-2 sm:px-4 py-3">Product</th>
                      <th className="px-2 sm:px-4 py-3">Details</th>
                      <th className="px-2 sm:px-4 py-3 text-right">Qty</th>
                      <th className="px-2 sm:px-4 py-3 text-right">Unit Price</th>
                      <th className="px-2 sm:px-4 py-3 text-right">Total</th>
                    </tr>
                  </thead>
                  <tbody className="text-sm divide-y divide-gray-50">
                    {(data.items ? data.items : []).map(item => (
                      <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-2 sm:px-4 py-3 font-semibold text-gray-900">{item.product || item.name}</td>
                        <td className="px-2 sm:px-4 py-3 text-gray-500 text-xs font-medium">Size: {item.size || "N/A"} | Color: {item.color || "N/A"}</td>
                        <td className="px-2 sm:px-4 py-3 text-right">{item.qty}</td>
                        <td className="px-2 sm:px-4 py-3 text-right">Rs. {item.unitPrice || item.price}</td>
                        <td className="px-2 sm:px-4 py-3 text-right font-semibold">Rs. {(item.qty * (item.unitPrice || item.price)).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className={isPage ? "lg:col-span-4 flex flex-col gap-6" : "h-full overflow-y-auto px-6 py-6 space-y-6"}>
            ...
            {data.type !== 'ONLINE' && (
              <section className={isPage ? "bg-transparent border border-gray-100/80 rounded-sm p-6 shadow-sm relative overflow-hidden group" : ""}>
  ...
  return (
    <>
      <div
        className={`fixed inset-x-0 bottom-0 top-16 z-[1000] flex justify-end md:inset-0 ${open ? "pointer-events-auto" : "pointer-events-none"
          }`}
        aria-hidden={!open}
      >
        <div
          className={`absolute inset-0 bg-black/40 backdrop-blur-[2px] ${open ? "animate-overlay-in" : "animate-overlay-out"
            }`}
          onClick={handleBack}
        />
        {detailPanelContent}
