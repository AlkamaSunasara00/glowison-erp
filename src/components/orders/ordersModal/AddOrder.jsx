...
const AddOrder = ({ open, onClose, isPage = false }) => {
  ...
  const [lineItems, setLineItems] = useState([{ id: Date.now(), name: "", sku: "", qty: 1, price: 0, pricePerUnit: "", size: "", color: "", unit: "inch", taxPercent: "", imageUrl: "", priceType: "Custom", retailPrice: 0, dealerPrice: 0 }]);
  ...
  const handleAddLineItem = () => {
    setLineItems([
      ...lineItems,
      { id: Date.now(), name: "", sku: "", size: "", color: "", qty: 1, price: 0, pricePerUnit: "", unit: "inch", taxPercent: "", priceType: "Custom", retailPrice: 0, dealerPrice: 0 }
    ]);
  };
  ...
  const handleLineItemChange = (id, field, value) => {
    setLineItems(prev => prev.map(item => {
      ...
      // Auto-update price if priceType changes
      if (field === 'priceType') {
  ...
  const handleCatalogSelect = (id, value, data) => {
    ...
    setLineItems(prev => prev.map(item => item.id === id ? {
      ...
      priceType: "Retail", // Default to retail when selected from catalog
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className={isPage ? "flex flex-col w-full h-full" : "flex h-full min-h-0 flex-col"}>
      <div className={isPage ? "border-b border-gray-200 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4" : "shrink-0 border-b border-gray-200 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"}>
        <div className="flex items-center justify-between w-full sm:w-auto gap-4">
          ...
          {!isPage && (
            <button type="button" onClick={handleClose} className="sm:hidden shrink-0">
        ...
        {isPage ? (
          <div className="flex flex-row items-center gap-3 w-full sm:w-auto mt-2 sm:mt-0">
            <Button variant="outline" type="button" onClick={handleClose} disabled={isSubmitting} className="flex-1 sm:flex-none justify-center">Cancel</Button>
            <Button variant="solid" type="submit" isLoading={isSubmitting} disabled={isSubmitting} className="flex-1 sm:flex-none justify-center">Create Order</Button>
          </div>
        ) : (
          <button type="button" onClick={handleClose} className="hidden sm:block shrink-0">
            <Icons name="X" size={18} className="text-gray-500 hover:text-gray-700" />
          </button>
        )}
      </div>
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 top-16 z-[1000] flex items-start justify-center p-4 md:inset-0 md:items-center md:p-6 ${open ? "pointer-events-auto" : "pointer-events-none"
        }`}
      aria-hidden={!open}
    >
      <div
      ...
      <div
        className={`relative flex h-full w-full max-w-4xl flex-col overflow-hidden rounded-md border border-gray-200 bg-white shadow-lg md:h-auto md:max-h-[90vh] ${open ? "animate-modal-in" : "animate-modal-out"
          }`}
      >
        {formContent}
      </div>
