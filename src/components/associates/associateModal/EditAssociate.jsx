...
const EditAssociate = ({ open, onClose, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        category: initialData.category || "INSTALLER",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await api.put(`/associates/${initialData.id}`, formData);
      toast.success('Associate updated successfully');
      onClose();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to update associate');
    } finally {
      ...
      >
        <form onSubmit={handleSubmit} className="flex h-full min-h-0 flex-col">
          <div className="shrink-0 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-gray-900">Edit Associate</h2>
              <p className="mt-1 text-sm text-gray-500">Update associate details.</p>
            </div>
          ...
          <div className="shrink-0 border-t border-gray-200 bg-gray-50/50 px-6 py-4 flex justify-end gap-3">
            ...
            <Button variant="solid" type="submit" isLoading={isSubmitting} disabled={isSubmitting}>Update Associate</Button>
          </div>
        </form>
      </div>
    </div>
